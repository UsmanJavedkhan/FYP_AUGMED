// list of users with role + active toggles + delete
  ...
  return (
    <article className="bg-white border border-[#e4e9f0] rounded-[20px] p-6 shadow-[0_10px_40px_-18px_rgba(15,23,42,0.2)]">
      ...
      <div>
        {users.map((u) => (
          <div
            className="grid grid-cols-[1.4fr_0.9fr_0.9fr_auto] gap-3.5 items-center px-4 py-3.5 border-b border-[#e4e9f0] last:border-b-0 text-[0.88rem]"
            key={u.id}
          >
            {/* user info */}
            <div>
            ...
            <select
              ...
              className="appearance-none cursor-pointer inline-flex px-2.5 py-1 rounded-full text-[0.72rem] font-semibold capitalize bg-[#f3f6fa] border border-[#e4e9f0] text-[#0f172a] disabled:cursor-not-allowed"
            >
              {ROLES.map((r) => (
